...
export default function ZeladorSettings() {
  ...
  const handleCropComplete = useCallback(async (croppedBlob: Blob) => {
    ...
    try {
      ...
      toast({ title: "Erro", description: error.message || "Não foi possível atualizar a foto.", variant: "destructive" });
    } finally {
      setUploading(false);
      if (fileInputRef.current) fileInputRef.current.value = "";
    }
  ...
  const handleRemoveAvatar = async () => {
    ...
    try {
      ...
      toast({ title: "Erro", description: error.message || "Não foi possível remover a foto.", variant: "destructive" });
    } finally {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate("/zelador")}>
        ...
        <div className="grid gap-6 md:grid-cols-2">
            ...
            <CardContent className="space-y-6">
              <div className="flex flex-col items-center">
                {showCropper && previewImage ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-muted-foreground mt-3">Clique para alterar a foto</p>
                    ...
                    {profileInfo?.avatar_url && (
                      ...
                      >
                        {removingAvatar ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Trash2 className="w-4 h-4 mr-1" />}
                        Remover foto
                      </Button>
                    )}
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label>Nome</Label>
                  <Input value={profileInfo?.full_name || ""} disabled className="bg-muted" />
                </div>
                <div className="space-y-2">
                  <Label>E-mail</Label>
                  <Input value={profileInfo?.email || ""} disabled className="bg-muted" />
                </div>
                <div className="space-y-2">
                  <Label>Telefone</Label>
                  <Input value={profileInfo?.phone || "Não informado"} disabled className="bg-muted" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={handleChangePassword} disabled={isChangingPassword || !newPassword || !confirmPassword} className="w-full">
                {isChangingPassword ? (
