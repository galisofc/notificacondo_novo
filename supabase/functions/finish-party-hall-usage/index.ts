...
serve(async (req) => {
  ...
  try {
    ...
    await supabase.from('edge_function_logs').insert({
      id: logId,
      function_name: 'finish-party-hall-usage',
      status: 'running',
    ...
    const { data: pauseStatus } = await supabase
      ...
      .select('paused')
      .eq('function_name', 'finish-party-hall-usage')
      .single();
    ...
    console.log(`Looking for in_use bookings for today or earlier: ${today}`);

    // Find all bookings that are in use for today or past days (handles missed cron runs)
    ...
    if (!bookings || bookings.length === 0) {
      console.log('No in_use bookings found for today');
      ...
      await supabase
        ...
        .update({
          ...
          duration_ms: Date.now() - startTime.getTime(),
          result: { 
            message: 'No in_use bookings found for today',
            date: today,
            bookingsProcessed: 0
          },
        })
    ...
    console.log(`Found ${bookings.length} in_use bookings for today`);
    ...
    console.error('Error in finish-party-hall-usage:', error);
